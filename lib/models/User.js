...
User.prototype.saveStrategy = function () {
  if (!this.id || !this.service || !this.service_user_id) {
    return db.err("Attempting to save a strategy with insufficient data");
  }
...
User.prototype.destroy = function () {
  if (!this.id) return db.err("Attempting to destroy an unsaved user");
    ...
    if (user.email) {
      return User.findBy({ email: user.email }).then(function (existingUserInfo) {
        if (existingUserInfo) {
          ...
          return user.saveStrategy().then(function () {
            return user;
          });
        }
...
User.update = function (user) {
  if (!user.id) return db.err("Attempting to update an unsaved user");
